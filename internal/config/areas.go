...
func validateAreaNodes(areas []AreaConfig, parent string) error {
	...
	for i, area := range areas {
		...
		// INTERIOR whitespace is permitted, a deliberate asymmetry with the check
		// above. The same reasoning applies — `nibs list --area "Web UI"` works
		// because a shell quotes it, while the web query box has no quoting and
		// tokenizes on whitespace, so `area:Web UI` splits and the tail lands in
		// free text. The box therefore withholds such a path from its completions
		// rather than offering one it cannot accept.
		//
		// It is NOT refused here: this function runs on every load, so tightening
		// it would make a config that is valid today fail outright, and areas.yml
		// is a stored file this project keeps readable across versions. Closing the
		// gap properly means quoting in the query grammar, a grammar-wide change
		// affecting every field.
