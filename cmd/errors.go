...
// pre-checks no id, so an unknown id arrives at mutationErrCode's own checks and
// nowhere else. Its sentinel branch is what gives such an id exit 3 rather than
// the concrete-type checks' exit 2, and cmd/mv_test.go's
// TestMvUnknownIdIsNotFound is what holds it there.
//
