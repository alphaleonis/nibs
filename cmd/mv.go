...
selects the milestone-queue scope for the after/before/first grammar; it takes
no --parent (assign with 'nibs set <id> --milestone' instead).`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// A queue move is single-nib too: the bulk reorders are sibling-block
		// operations in the parent scope, and a block move in a queue has no
		// grammar here. (--queue with --children-of or --parent is refused by
		// the Cobra mutex below.)
		if mvQueue && len(args) >= 2 {
			return cmdError(mvJSON, output.ErrValidation,
		...
		if mvQueue && !hasPosition {
			return cmdError(mvJSON, output.ErrValidation,
				"--queue needs a position: --after, --before or --first (the anchor must be in the same milestone queue)")
		}
		...
		scope := model.OrderScopeParent
		if mvQueue {
			scope = model.OrderScopeMilestone
		}
