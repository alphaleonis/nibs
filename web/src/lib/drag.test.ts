...
describe("DragState", () => {
  ...
  it("setDropTarget carries the accepted plan's sentence and kind", () => {
    const state = new DragState();
    state.startDrag(["nib-001"]);
    state.setDropTarget("nib-002", "before", true, {
      kind: "position",
    ...
    expect(state.dropAccepted).toEqual({
      kind: "position",
      label: "Reorder in the Q3 Launch queue",
      region: { axis: "milestone", milestoneId: "M1" },
    });
  ...
  it("carries an assignment, which names a sentence and no region at all", () => {
    const state = new DragState();
    state.startDrag(["nib-001"]);
    state.setDropTarget("nib-002", "reparent", true, { kind: "assign", label: "Move to the web area" });
    expect(state.dropLabel).toBe("Move to the web area");
    expect(state.dropAccepted).toEqual({ kind: "assign", label: "Move to the web area" });
  });
  ...
  it("a target set with no accepted plan carries neither", () => {
    ...
    expect(state.dropAccepted).toBeNull();
  });
  ...
  it("clearDropTarget and endDrag drop the accepted plan too", () => {
    ...
    expect(cleared.dropAccepted).toBeNull();
    ...
    expect(ended.dropAccepted).toBeNull();
  });
