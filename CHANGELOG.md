...
- **Work is assigned to a milestone and ordered within its queue** — `nibs set --milestone` assigns (never a nib and one of its ancestors both), `nibs list --milestone`/`--backlog` read a queue and the unscheduled remainder, `nibs mv --queue` repositions one entry by rewriting one file, and a write that leaves work queued ahead of something still blocking it says so, whether it came from the CLI or over GraphQL.
...
- **`nibs check` names a `milestone:` assignment whose target is not a milestone** — such a nib confers no membership and sits in the backlog while its `milestone:` field still reads back the bad target, which nothing reported before; `--fix` refuses, since choosing the right milestone is not provable intent.
- **BREAKING: assigning open work to a finished milestone is refused, and a milestone holding assignments cannot stop being one** — assigning open work to a milestone closed as completed or scrapped is refused (one closed as deferred still accepts it, and already-closed work may still be recorded against a finished wave), and changing a milestone's type while nibs are assigned to it is refused because every one of those assignments would name a nib that is no longer a milestone.
...
- **Work is placed in an area from a vocabulary the project declares** — an `areas:` block in the store's `areas.yml` declares a nested tree, `nibs set --area`/`nibs new --area` place a nib in one, `nibs list --area` reads an area together with everything declared beneath it, `nibs area list|add|set|rm` prints and edits the vocabulary, and a running `nibs serve` picks an edit up rather than needing a restart.
...
- **Areas are declared, edited and retired over the API** — a project that declares no vocabulary yet gets one from its first declaration, an area's name, description and color are changed in one call, and the work assigned beneath a renamed or retired area is reassigned or unassigned as the call directs.
...
- **BREAKING: the roadmap reads as each milestone's queue** — a milestone's section is one Markdown list in `milestone_order` with loose work interleaved among the epics and each epic's items nested beneath it in place of the `### Epic:` heading blocks, the backlog is in tree order under a `## Backlog` heading, and `--json` carries one `queue` array per milestone in place of the `epics`/`other` split, with the top-level `unscheduled` key renamed `backlog` to match the name `nibs list --backlog` already used.
...
- **The roadmap no longer omits outstanding work** — it expanded epics alone, so a feature or bug with children rendered as one bare bullet while its children appeared in no section at all, and an open container whose children had all closed vanished entirely even while `nibs next` was offering it as the thing to do.
- **The roadmap's backlog now shows exactly the work outside every milestone** — items under a milestone hidden by a `--status` filter no longer leak into the backlog, and a work item whose parent link names no nib now appears there instead of vanishing from the roadmap entirely.
- **`nibs plan` no longer writes to the store when you read it** — it backfilled a missing order key to disk as a side effect of listing, leaving a clean `.nibs` repo dirty and restamping `updated_at`.
