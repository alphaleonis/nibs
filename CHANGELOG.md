...
- **BREAKING: the roadmap reads as each milestone's queue** — a milestone's section is one Markdown list in `milestone_order` with loose work interleaved among the epics and each epic's items nested beneath it in place of the `### Epic:` heading blocks, the backlog is in tree order under an `## Unplanned` heading, and `--json` carries one `queue` array per milestone in place of the `epics`/`other` split.
