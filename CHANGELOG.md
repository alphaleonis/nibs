...
- **Syntax highlighting in the filter box**, colored per token: the field name and its punctuation read as structure, values take the accent, free text stays muted, and a value that is not legal for its field gets a red wavy underline. An invalid token is preserved rather than discarded — results reflect the valid part while the typo survives blur, a dropdown edit and a round-trip through the URL.
