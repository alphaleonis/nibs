...
Because the config travels with the data, `--nibs-path <dir>` and `NIBS_PATH` apply **that store's** prefix, id_length and defaults — pointing nibs at another project reads it under its own vocabulary, not the cwd project's. `--config <file>` names a store's `config.yml` and resolves the store as that file's containing directory, so it is redundant with `--nibs-path`; pointed at a pre-layout `.nibs.yml` it is refused, because that file sits *beside* the store and its directory is the project. Whichever route names the store, the directory must look like one (named `.nibs`, or holding `config.yml`/`data/`/`archive/`, or the pre-layout shape) — otherwise a path aimed one level too high would let `nibs migrate` loose on the project tree.
