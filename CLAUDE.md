...
**Run `task` from Git Bash, not PowerShell.** The Taskfile shells out through `bash scripts/…`, and in PowerShell `bash` resolves to `C:\Windows\system32\bash.exe` — the WSL launcher — so the entire Go suite runs on **Linux**, against `/mnt/<drive>/…`, using WSL's Go instead of the `mise.toml` pin. The tell is the skip tally: on Linux every capability exists, so nothing skips and `internal/testskip` prints nothing at all, and an empty tally reads exactly like "every guard ran" when in fact none of them ran on Windows. The one loud symptom is the web lane, where WSL's node cannot load the Windows-native binaries in `web/node_modules` (rolldown's "Cannot find native binding", surfacing as one `svelte-check` error per file in files nobody touched). That noise is the only thing separating a wrong-platform run from a silent one (nibs-ehym).
