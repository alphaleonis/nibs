...
- **`internal/fsutil/`** - `AtomicWriteFile`: temp file plus rename, mode-preserving, with a best-effort directory fsync. Stdlib-only so both `internal/config` and `internal/nibcore` can share one writer
