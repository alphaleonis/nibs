...
- `task codegen` - Regenerate GraphQL code: Go server (gqlgen, `go generate ./...`) + web client types (graphql-codegen client preset). Runs `go:codegen` + `web:codegen`. The Go pass also regenerates `web/src/lib/generated/vocabulary.ts` (statuses with roles, types, hierarchy rules, priorities, estimates — from `internal/webvocab`); the committed output is byte-pinned by `TestGeneratedVocabularyIsFresh`, so a vocabulary change fails `task test` until regenerated and committed. It also regenerates `web/src/lib/generated/membershipContract.ts` (from `internal/membershipcontract`) — the Go↔TS parity contract for the milestone-membership rule, byte-pinned the same way by `TestGeneratedMembershipContractIsFresh`.
...
Two extra prerequisites, and both failure modes produce a run that looks like a verdict on Windows when it is not one.

**Install a C toolchain, or `task test` cannot finish.** `scoop install main/gcc` (mingw-w64) is enough; `CGO_ENABLED` flips from 0 to 1 on its own once a compiler is on PATH, because that is how Go picks the default, so there is nothing to configure afterwards. Without it `test:race` fails with `go: -race requires cgo`, and because that step runs *before* `web:check` and vitest, those two lanes never execute either — the message names cgo while the coverage actually lost is three lanes wide.

The compiler is needed even though the race detector ships **prebuilt** (`race_windows.syso` is already inside the Go installation, built from LLVM's ThreadSanitizer). Nothing is compiled from source. cgo is required for two other things: TSan intercepts the C library's thread creation, so `runtime/race/race.go` pulls in `runtime/cgo` to route Go's threads through it rather than raw syscalls the detector cannot see; and linking a foreign `.syso` into a Go binary hands the final link to the system linker, which needs the C runtime import libraries (`libmsvcrt.a`, `libmingwex.a`). macOS is exempt from the requirement precisely because it already creates threads through system libraries and ships the symbol information separately — `cmd/go/internal/work/init.go` special-cases it. MSVC cannot serve in gcc's place: cgo drives the compiler with GCC-style flags, and neither `cl.exe` nor `clang-cl` accepts them.

The lane is around 5x slower here than on Linux on the same hardware (~92s versus ~17s; `internal/graph` alone 63s versus 4s), which is why CI runs the detector on its ubuntu leg only.

**Run `task` from Git Bash, not PowerShell.** The Taskfile shells out through `bash scripts/…`, and in PowerShell `bash` resolves to `C:\Windows\system32\bash.exe` — the WSL launcher — so the entire lane would run on **Linux**, against `/mnt/<drive>/…`, using WSL's Go instead of the `mise.toml` pin. This is now **refused rather than silently obeyed**: the Taskfile passes `--host-os {{OS}}` to `run-capped.sh`, which compares where Task ran against where it is itself executing and exits with an explanation. Start Task inside WSL if you want to test Linux on purpose — there `{{OS}}` is `linux` and nothing fires.

The refusal exists because the silent version was almost undetectable. The Go suite answers for Linux while looking like a Windows run, and the skip tally comes back *empty* — on Linux every capability exists, so nothing skips and `internal/testskip` prints nothing at all, which reads exactly like "every guard ran". The only loud symptom was the web lane, where WSL's node cannot load the Windows-native binaries in `web/node_modules`. The marker is an argument rather than an environment variable for a reason worth knowing before changing it: a Windows env var is invisible inside WSL unless named in `WSLENV`, and Task's `env:` cannot set `WSLENV` when the parent shell already has one — which Windows Terminal does by default (nibs-ehym).

**Symlink guards need Developer Mode or an elevated shell.** `os.Symlink` commonly requires one or the other, and fifteen fixtures skip without it. They are counted rather than silent — read the tally `task test` prints — but a run reporting symlink skips has not exercised the store-resolution refusals those fixtures are the only coverage for. `hostile filenames`, `named pipes` and `unreadable paths` skips are expected on Windows and are not fixable: those capabilities do not exist there.
...
- **`internal/fsutil/`** - Two writers over one temp-file-plus-rename mechanism, mode-preserving, stdlib-only so both `internal/config` and `internal/nibcore` can share them. `AtomicWriteFile` ends with a best-effort fsync of the containing directory; `AtomicWriteFileDeferDirSync` omits it and returns that directory instead, for a bulk loop that would otherwise fsync the same directory once per file. The deferred variant's caller owes `SyncDirFn` one call per **distinct** directory it collected — a loop can span `data/`, its subdirectories and `archive/` — and owes them on the error path too, since an aborted batch has already committed its earlier renames (`fsutil.DirSyncBatch` is that collector — declare one, `defer` its `Flush`, and pass it by pointer)
- **`internal/safetext/`** - The rendering boundary for text nibs did not write: `Strip` for one scalar in a message, `Writer` for a sink that carries only such messages. Stdlib-only so every layer can reach it
...
Project config lives INSIDE the store, at `.nibs/config.yml`. Commands walk up from the cwd in **one pass** and bind to the **nearest** nibs marker of *either* kind — a `.nibs` directory (a store) or a pre-layout `.nibs.yml` (a project with no store yet) — then read the config from within the store. Within one directory the store wins, since the pre-layout default shape is `.nibs.yml` beside a `.nibs` store and binding to the store is what lets the migration gate speak for it. Two independent walks cannot answer "which is nearer" without re-deriving depth from the paths they return, and nothing did. Key settings: `nibs.prefix` (ID prefix like "myproj-"), `nibs.id_length`, `nibs.require_if_match` (optimistic concurrency).

Because the config travels with the data, `--nibs-path <dir>` and `NIBS_PATH` apply **that store's** prefix, id_length and defaults — pointing nibs at another project reads it under its own vocabulary, not the cwd project's. `--config <file>` names a store's `config.yml` and resolves the store as that file's containing directory, so **given alone** it is redundant with `--nibs-path`; its basename must literally be `config.yml` (any other name splits the store from its vocabulary inside one flag), and pointed at a pre-layout `.nibs.yml` it is refused, because that file sits *beside* the store and its directory is the project. `--config` **cannot be combined** with `--nibs-path` or `NIBS_PATH` — together they decouple a store from its vocabulary (the store came from one, the config from the other), and since ids derive from filenames a nib written that way is permanently misnamed, not merely mislabeled.

Every route — the three that name a store explicitly and the upward walk that discovers one — puts the directory through **one** evidence check, and it must carry evidence a nibs store would leave: it is a **real directory** named `.nibs`, or it holds a `config.yml` that *parses* as a nibs config, or a pre-layout `.nibs.yml` **in its parent** *names* it through the retired `nibs.path` key **and** the directory either holds a markdown file in the shape `nib.Render` writes **or is empty** — a store `nibs init` created but never wrote to legitimately holds nothing, and that is the only exemption: a populated directory with no nib in it is refused, whatever else it holds. A `.nibs` that is a **symlink** is deliberately outside the name clause and falls through to the other two: the name is all that clause trusts, and for a link the name and the directory it leads to are different things — a committed `.nibs -> /outside` was bound as the store on every route (including a bare `nibs list` with no flag at all) and `nibs migrate` planned to sweep that tree into `<project>/.nibs`. So `.nibs -> ~/sync/proj-nibs` holding a real `config.yml` still resolves, and a link to anything else is refused. This is an evidence rule, not a containment one — bounding *where* a link may lead would also refuse the legitimate off-repo store. The rule has a second half in `nibs init`, and the two are load-bearing together: init refuses to CREATE a store through a `.nibs` link (`MkdirAll` follows one, so the store would land at the far end and that tree would then parse as a store), which is what makes the resolution refusal safe to prescribe `nibs init` at all. Setting the store up off-repo is `nibs init --nibs-path <dir> --prefix <p>` and then linking at it — `--prefix` because a store outside the project derives its prefix from its own parent. The naming half of the `nibs.path` clause is also checked on the **resolved** path (`filepath.EvalSymlinks`): the comparison is otherwise lexical, so a committed `store -> /outside` symlink satisfied "an immediate subdirectory of the project" while pointing anywhere, and the relocation then swept that whole tree into `<project>/.nibs`. A link that stays inside the project is still accepted — this bounds *where* a config may authorize, it does not ban links. Being named something a store also uses — `data/`, `archive/`, a file called `config.yml` — is deliberately **not** evidence; otherwise a path aimed one level too high lets `nibs migrate` loose on the project tree (a Hugo site's `data/` directory was enough). Nor is the naming config alone: a cloned repository writes its own `.nibs.yml`, and pre-layout `nibs init` never wrote a `nibs.path` other than `.nibs`, so an unaccompanied pointer is always hand-authored. The corroborating half of that clause asks for the whole shape `nib.Render` writes — the id comment on the first line inside the fence, then `version`, `title` and `status`, the three keys it never omits — and one file passing is enough. A lone `status:` used to be the bar and ordinary content reached it, since note vaults and docs sites track a page's own state that way; a `notes/` directory was renamed to `.nibs` and every file in it rewritten as a nib render. **The shape is still not provenance**: anything that knows it can wear it, and whoever writes `.nibs.yml` writes the markdown beside it, so against a repository that chose its `nibs.path` deliberately this proves nothing and no stricter shape would. What answers that case is the containment check above, which confines a hostile config to the checkout it ships in.

This check has **two** failure directions and both are load-bearing. It **refuses** every `nibs.path` shape that does not resolve to an immediate subdirectory of the config's own directory — nested (`docs/nibs`), absolute-and-outside, `.`, `..`, a symlink leading out of the project — because it is an authorization decision (what it accepts, `nibs migrate` may move and rewrite) and because the migration engine derives the project from the store's parent. Those shapes are still served: the discovery message prints a *manual* remedy for them (create `<project>/.nibs`, move the nib files in, remove the `nibs.path` key, then `nibs migrate`) and prints the `nibs migrate --nibs-path <dir>` command **only** for shapes the check accepts, so the tool never prescribes a command it refuses. Evidence that exists but cannot be read — a `config.yml` over `config.MaxConfigBytes`, an unreadable `.nibs.yml` — is a third answer, reported as "cannot tell" rather than as absence, because the absence message says to run `nibs init` here.

The `nibs.path` key is retired: a config still carrying it is a hard error naming `nibs migrate`. Migrating a pre-layout project (`.nibs.yml` beside the store, nib files at the store root) is the `layout` step of `nibs migrate`, which every command refuses until it has run. That step also **moves the store itself** into `<project>/.nibs` when `nibs.path` named an immediate subdirectory of the project — a store must *be* at `.nibs` to be discoverable, so converting one in place would produce an unusable project. Any other `nibs.path` shape is the manual remedy above, not an automatic relocation.

A nearer pre-layout project **shadows** an ancestor store rather than being walked past: the walk stops at the `.nibs.yml`, and since a project in that shape has no store to bind to, every command refuses with the migration remedy (naming the ancestor too, since the reader may have watched commands answer from it). Binding past it was a data-mutation bug — `nibs migrate` in a nested pre-layout sub-project moved and rewrote the **ancestor's** nib files while the sub-project stayed untouched.
...
**The `nibs` on PATH is the RELEASED build, and it is not this branch.** You track your work with it — this repo's `.nibs` store requires it, and keeping the tracker off the code under development is what stops a half-finished refactor from taking away the ability to record work. Do not "fix" that separation by migrating the store. The cost is that `nibs cheat`, `nibs catalog` and `nibs <cmd> --help` run from PATH describe a CLI that may be hundreds of commits behind this worktree, missing whole commands and flags. Never cite them as evidence about code you are changing, and never copy a flag grammar out of them into new documentation — an already-wrong line is exactly how the last such defect propagated (a `--first <anchor>` form copied from a neighbouring help line, where `--first` is a bool that takes no anchor). To learn what this branch does: `task build`, then run `./nibs` against a throwaway copy of `testdata/fixtures/sample-project`. `scripts/prime-hook.sh` states this at session start, with the current commit drift measured.

**Execute every factual claim you write about CLI behavior before committing it.** Prose is the only artifact here with no compiler and no test behind it, so a plausible-sounding sentence passes every gate and is then believed — by agents told not to guess syntax and to STOP on any error, which turns a wrong grammar into a halted run. `TestDocumentedFlagShapesTakeValues` catches the mechanical half (a value-less flag documented as taking one); the rest is on you, and it is one shell command per claim.
...
- **One sentence.** Two only for a BREAKING change needing a migration note. Rationale, alternatives considered and verification evidence belong in the nib and the commit message. Evidence hides in plausible-looking forms — "22 screens were diffed against the pre-migration build", "38 nibs on the fixture, where the field previously reported 67", "a 100 KB id returned an 8.9 MB response" — all describing what was done to verify rather than what a user gets.
...
**Two more the second trim added**, after the first pass shortened entries but left these:

- **One entry per change, not per facet.** A feature split across five bullets is one entry; in `Fixed`, the knock-on effects of a single bug fold into the fix they follow from. If consecutive entries all elaborate the first, they are one entry. The other three tests govern how *long* an entry is, none of them how many entries a change gets — which is how one feature occupied five bullets while each bullet passed the one-sentence test.
- **The bold lead is usually the whole entry.** Token grammars, flag lists and field enumerations are reference material `nibs catalog`, `nibs cheat` and the in-app help already carry.

Match the terseness of `v0.7.0` and earlier, not the section above it. At merge time re-read the **whole** section, not only your own entry: later work in the same cycle falsifies earlier ones, and version numbers are the usual casualty — a v0.8.0 security entry credited goldmark `1.7.13 → 1.7.17` after a later bump had already carried it to 1.8.5.
...
- **A skip that the environment forces must be counted, not just taken.** `go test ./...` prints `ok <pkg>` and discards that package's output when everything passes or skips, so a run where a whole family of guards never executed looks exactly like a clean one — and CI's windows leg is that shape, on a platform where `os.Symlink` commonly needs elevation. Take such a skip through `internal/testskip` (`testskip.SymlinkUnavailable(t, err)`, or `testskip.Unavailable` for another capability): each package's `TestMain` then reports the tally, `NIBS_SKIP_REPORT` collects it into a file a CI step prints, and setting the capability's env var (`NIBS_REQUIRE_SYMLINKS=1`, which CI's Linux leg does) turns the skip into a failure where the capability is known to be present. `TestSymlinkSkipsAreEnrolled` fails on a hand-rolled symlink skip.
